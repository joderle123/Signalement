...
function renderArbeitsblaetter(themaId) {
  ...
  const fachkraft = typeof FACHKRAFT_ANLEITUNGEN !== 'undefined' ? (FACHKRAFT_ANLEITUNGEN[themaId] || null) : null;

  if (blaetter.length === 0 && aktivitaeten.length === 0 && interventionen.length === 0 && !modul && !fachkraft) return '';

  const hasModul = modul || aktivitaeten.length > 0 || interventionen.length > 0;
  const hasFachkraft = !!fachkraft;
    ...
    <div style="margin-bottom:20px;">
      <div class="panel-tabs" id="panel-tabs-${themaId}">
        ...
        ${hasFachkraft ? `<button class="panel-tab" onclick="switchPanelTab('${themaId}','fk')">
          🎓 Fachkraft-Anleitung
          <span style="font-size:10px;font-weight:400;opacity:0.65;display:block;margin-top:1px;">Ebene 3 · Fachkraft</span>
        </button>` : ''}
      </div>
      ...
      <div id="pt-fk-${themaId}" class="panel-tab-content" style="display:none;">
        <div style="background:#FDF2F8;border:1.5px solid #FBCFE8;border-radius:8px;padding:9px 12px;margin-bottom:14px;font-size:11px;color:#9D174D;line-height:1.5;">
          <strong>Fachkraft-Anleitung (Ebene 3)</strong> · Hintergrundwissen & Handlungsempfehlungen<br>
          <span style="opacity:0.75;">Theoretische Grundlagen, ICD-Einordnung und konkrete Gesprächsstrategien für die Fachkraft.</span>
        </div>
        <a href="fachkraft-module/${fachkraft.datei}" target="_blank"
           style="display:flex;align-items:center;gap:10px;padding:11px 14px;margin-bottom:6px;
                  background:#FDF2F8;border:1.5px solid #F9A8D4;border-radius:6px;
                  text-decoration:none;color:#9D174D;font-size:12px;font-weight:600;">
          <span style="font-size:18px;">🎓</span>
          <div style="flex:1;">
            <div>${fachkraft.titel}</div>
            <div style="font-size:10px;font-weight:400;opacity:0.7;margin-top:2px;">Fachkraft-Modul · Druckbar · PDF-optimiert</div>
          </div>
          <span style="font-size:11px;opacity:0.7;">Öffnen →</span>
        </a>
      </div>` : ''}
...
function switchPanelTab(themaId, tab) {
  ...
  if (tabs) {
    const tabButtons = tabs.querySelectorAll('.panel-tab');
    // Build ordered list of tab keys matching the rendered buttons
    const tabKeys = ['ab'];
    if (document.getElementById(`pt-tm-${themaId}`)) tabKeys.push('tm');
    if (document.getElementById(`pt-fk-${themaId}`)) tabKeys.push('fk');
    tabButtons.forEach((btn, i) => {
      btn.classList.toggle('active', tabKeys[i] === tab);
    });
  }
}
